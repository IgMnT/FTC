...
with tab1:
    ...
    with st.container():
        ...
        with col1:
            ...
            df_ent = (df1.loc[:, ['Delivery_person_ID','City', 'Time_taken(min)']]).groupby(['City', 'Delivery_person_ID']).mean().sort_values(['City', 'Time_taken(min)'], ascending=False).reset_index()
            df_aux = df_ent.loc[df_ent['City'] == 'Metropolitan', :].head(10)
            df_aux2 = df_ent.loc[df_ent['City'] == 'Urban', :].head(10)
            df_aux3 = df_ent.loc[df_ent['City'] == 'Semi-Urban', :].head(10)
            df_ent_rap = pd.concat([df_aux, df_aux2, df_aux3]).reset_index(drop=True)
            st.dataframe(df_ent_rap)
        with col2:
            st.markdown('#### Top entregadores mais lentos')
            df_ent = (df1.loc[:, ['Delivery_person_ID','City', 'Time_taken(min)']]).groupby(['City', 'Delivery_person_ID']).mean().sort_values(['City', 'Time_taken(min)'], ascending=True).reset_index()
            df_aux = df_ent.loc[df_ent['City'] == 'Metropolitan', :].head(10)
            df_aux2 = df_ent.loc[df_ent['City'] == 'Urban', :].head(10)
            df_aux3 = df_ent.loc[df_ent['City'] == 'Semi-Urban', :].head(10)
            df_ent_rap = pd.concat([df_aux, df_aux2, df_aux3]).reset_index(drop=True)
